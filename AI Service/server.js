...
app.get("/health", async (req, res) => {
  ...
  try {
    if (PROVIDER === "ollama") {
      ...
      // Optional: you *could* do a tiny generateContent ping, but that can hit quota.
    } else {
      dep = "n/a";
    }
  } catch {
...
async function geminiJson(prompt, fallbackObj) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (is429 && attempt === 1) {
        await sleep(2000); // quick backoff; you can increase if needed
        continue;
      }
...
`;
  ...
  res.json({
    ...
    error: parsed.error, // helpful when Gemini quota/rate-limit happens
  });
});
